...
int VideoDecoder::read_device_info() {
	...
	SPDLOG_TRACE(DECODER_LOGGER "Trying to read device info from socket {} ", this->socket);
	int bytes_read = recv(this->socket, device_info_data, buf_size, 0);
	SPDLOG_TRACE(DECODER_LOGGER "{} bytes received from socket {} ", bytes_read, this->socket);
...
int VideoDecoder::init_decoder() {
	...
	if (!this->packet_buffer) {
		SPDLOG_ERROR(DECODER_LOGGER "No enough memory for packet buffer");
		return -1;
	}
	this->active_data = (char*)malloc(cfg->video_packet_buffer_size_kb * 1024);
	if (!this->active_data) {
		SPDLOG_ERROR(DECODER_LOGGER "No enough memory for active_data");
		return -1;
	}
	enum AVCodecID h264 = AV_CODEC_ID_H264;
	const AVCodec *codec = (AVCodec *)avcodec_find_decoder(h264);
	if (!codec) {
		SPDLOG_ERROR(DECODER_LOGGER "Could not find h264 codec");
		return -1;
	}
	AVCodecContext* codec_context = avcodec_alloc_context3(codec);
	if (!codec_context) {
		SPDLOG_ERROR(DECODER_LOGGER "No enough memory for codec_context");
		return -1;
	}
	if (avcodec_open2(codec_context, codec, NULL) != 0) {
		SPDLOG_ERROR(DECODER_LOGGER "Failed to open codec");
		avcodec_free_context(&codec_context);
		return -1;
	}
	AVCodecParserContext* codec_parser_context = av_parser_init(h264);
	if (!codec_parser_context) {
		SPDLOG_ERROR(DECODER_LOGGER "Failed to init paser context");
		avcodec_free_context(&codec_context);
		return -1;
	}
	...
	if (!packet) {
		SPDLOG_ERROR(DECODER_LOGGER "Failed to init packet");
		avcodec_free_context(&codec_context);
...
int VideoDecoder::read_video_header(struct VideoHeader* header) {
	...
	SPDLOG_TRACE(DECODER_LOGGER "Trying to read video header({} bytes) from {} into {} ", H264_HEAD_BUFFER_SIZE, this->socket, (uintptr_t)header_buffer);
	int bytes_received = recv(this->socket, header_buffer, H264_HEAD_BUFFER_SIZE, 0);
	if (bytes_received != H264_HEAD_BUFFER_SIZE) {
		SPDLOG_ERROR(DECODER_LOGGER "Error, Read {}/{} for video header", bytes_received, H264_HEAD_BUFFER_SIZE);
		return 1;
	}
...
int VideoDecoder::recv_network_buffer(int length, char* buffer, char* chunk) {
	...
	while (read_total < length) {
		...
		SPDLOG_TRACE(DECODER_LOGGER "Receiving {}/{} from network for socket {}", read_total, length, this->socket);
...
int VideoDecoder::prepare_packet(uint64_t pts, int length) {
	...
	SPDLOG_TRACE(DECODER_LOGGER "is_config = {}, has_pending = {} for socket {}", is_config ? "yes" : "no", has_pending ? "yes" : "no", this->socket);
	if (is_config || has_pending) {
		int offset = 0;
		if (has_pending) {
			SPDLOG_TRACE(DECODER_LOGGER "Detected pending, offset will be {} ", this->pending_data_length);
			offset = this->pending_data_length;
		}
		else {
			SPDLOG_TRACE(DECODER_LOGGER "no pending data, saving received data to pending buffer for socket {}", this->socket);
		...
		if (offset > 0) {
			...
			SPDLOG_TRACE(DECODER_LOGGER "Existed pending data size = {}, current pending size = {}, final size={}, socket={}", this->pending_data_length, length, 
		...
		if (!is_config) {
			SPDLOG_TRACE(DECODER_LOGGER "Preparing active packet");
	...
	if (is_config) {
		SPDLOG_TRACE(DECODER_LOGGER "In configuring, will not call decoder for socket {}", this->socket);
		active_packet->data = NULL;
		av_packet_unref(active_packet);
		result = 1;
	}
...
image_size* VideoDecoder::get_image_size() {
	if (NULL == this->callback) {
		SPDLOG_TRACE(DECODER_LOGGER "no image size provider configured");
		return NULL;
	}
	if (0 == strlen(this->device_id)) {
		SPDLOG_TRACE(DECODER_LOGGER "no device id provided");
		return NULL;
	}
...
int VideoDecoder::rgb_frame_and_callback(AVCodecContext* dec_ctx, AVFrame* frame) {
	...
	if (NULL != configrued_size && configrued_size->width > 0 && configrued_size->height > 0) {
		...
		SPDLOG_TRACE(DECODER_LOGGER "Resizing image from {}x{} to {}x{}", width, height, target_width, target_height);
	}
	...
	std::lock_guard<std::mutex> lock_guard{ this->img_buffer_lock };
	SPDLOG_TRACE(DECODER_LOGGER "Encoding image to png format");
	if (cv::imencode(".png", image, *this->img_buffer)) {
		image.release();
		int img_size = this->img_buffer->size();
		SPDLOG_TRACE(DECODER_LOGGER "sending {} bytes to callback", img_size);
...
int VideoDecoder::decode_frames(uint64_t pts, int length) {
	...
	int status = 0;
	AVFrame* frame = NULL;
	SPDLOG_TRACE(DECODER_LOGGER "decode_frames pts={} length={} socket={}", pts, length, this->socket);
	...
	SPDLOG_TRACE(DECODER_LOGGER "Fetching codec parser context for socekt {}", this->socket);
	...
	if (parser_context->key_frame == 1) {
		...
		SPDLOG_TRACE(DECODER_LOGGER "Confiuring flags for socket {}", this->socket);
	}
	SPDLOG_TRACE(DECODER_LOGGER "Fetching codec context for socket {}", this->socket);
	AVCodecContext* codec_context = this->codec_ctx;
	SPDLOG_TRACE(DECODER_LOGGER "Sending packet for decoding, data pointer address is {} size={} socket={}", (uintptr_t)active_packet->data,
		active_packet->size, this->socket);
	result = avcodec_send_packet(codec_context, active_packet);
	if (result != 0) {
		reset_has_pending = TRUE;
		SPDLOG_ERROR(DECODER_LOGGER "Could not invoke avcodec_send_packet: {} socket={}", result, this->socket);
		active_packet->data = NULL;
		av_packet_unref(active_packet);
		goto end;
	}
	...
	while (status >= 0) {
		...
		if (status == 0) {
			SPDLOG_TRACE(DECODER_LOGGER "Got frame with width={} height={} socket={} ", frame->width, frame->height, this->socket);
		...
		if (has_pending && reset_has_pending) {
			SPDLOG_TRACE(DECODER_LOGGER "Reset has_pending=false for socket {}", this->socket);
			this->has_pending = FALSE;
		}
		return result;
}
int VideoDecoder::decode() {
	if (this->read_device_info()) {
		SPDLOG_ERROR(DECODER_LOGGER "Failed to read device info for socket {} ", this->socket);
		return 1;
	}
	if (this->init_decoder() != 0) {
		SPDLOG_ERROR(DECODER_LOGGER "Failed to init decoder for socket {} ", this->socket);
		return 1;
	}
	...
	while (*this->keep_running == 1 && keep_connection == 1) {
		...
		if (header_size <= 0) {
			keep_connection = 0;
			status = 1;
			SPDLOG_ERROR(DECODER_LOGGER "Failed to read header info from {}", this->socket);
			break;
		}
		...
		if (decode_status == -1) {
			SPDLOG_ERROR(DECODER_LOGGER "Bad status for decoding video from {}, will not continue", this->socket);
			break;
		}
	}
	return status;
