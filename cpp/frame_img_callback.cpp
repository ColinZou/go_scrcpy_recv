...
int frame_img_processor::callback_thread(device_frame_img_callback *callback_item) {
	...
	while (true) {
		...
		for (int i = 0; i < callback_item->handler_count; i++) {
            ...
            debug_logf(FIP_LOGGER "Invoking callback handler %p for device_id=%s callback param is %p\n", (uintptr_t)callback, 
        ...
        debug_logf(FIP_LOGGER "Setting status to CALLBACK_PARAM_SENT for %p\n", (uintptr_t) allocated_frame);
        allocated_frame->status = CALLBACK_PARAM_SENT;
        debug_logf(FIP_LOGGER "Put the frame %p back to %p\n", (uintptr_t) allocated_frame, (uintptr_t) frames);
...
void frame_img_processor::add(char *device_id, frame_callback_handler callback, char *token) {
    ...
    debug_logf(FIP_LOGGER "Trying to add frame image callback handler %p for device %s existing? %s \n", (uintptr_t)callback, 
...
void frame_img_processor::invoke(char *token, char* device_id, uint8_t* frame_data, uint32_t frame_data_size, int w, int h, int raw_w, int raw_h) {
    ...
    if (params == nullptr) {
        debug_logf(FIP_LOGGER "FATAL: Could not allocate a param for sending callback\n");
        return;
    }
    debug_logf(FIP_LOGGER "Current callback param is %p\n", (uintptr_t)params);
    // realloc ram
    if (params->buffer_size < frame_data_size) {
        free(params->frame_data);
        auto buffer_size = calc_buffer_size(frame_data_size, MAX_IMG_BUFFER_SIZE);
        debug_logf(FIP_LOGGER "Re-allocating %d bytes for fram cache\n", buffer_size);
        params->frame_data = (uint8_t*)malloc(buffer_size);
        params->buffer_size = buffer_size;
        debug_logf(FIP_LOGGER "Re-allocated %d bytes for fram cache\n", buffer_size);
    }
